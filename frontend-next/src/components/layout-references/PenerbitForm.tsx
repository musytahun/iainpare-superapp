...
import {
  ...
  CREATE_PENERBIT,
  UPDATE_PENERBIT,
} from "@/graphql/references/penerbit.graphql";
import { useRouter } from "next/navigation";

export default function PenerbitForm({ getPenerbit }: { getPenerbit: any }) {
  const router = useRouter();

  const [form, setForm] = useState({
    code: getPenerbit?.code || "",
    name: getPenerbit?.name || "",
  });

  const [createPenerbit] = useMutation(CREATE_PENERBIT, {
    refetchQueries: [{ query: GET_PENERBIT }],
    onCompleted: () => router.push("/references/penerbit"),
  });

  const [updatePenerbit] = useMutation(UPDATE_PENERBIT, {
    refetchQueries: [{ query: GET_PENERBIT }],
    onCompleted: () => router.push("/references/penerbit"),
  });
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm p-6">
      <h1 className="text-2xl font-bold text-slate-800 dark:text-white mb-6">
        {getPenerbit ? "Edit Penerbit" : "Tambah Penerbit"}
      </h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="pt-4 flex justify-end space-x-2">
          <button
            type="button"
            onClick={() => router.push("/references/penerbit")}
          ...
          >
            {getPenerbit ? "Simpan Perubahan" : "Tambah Data"}
          </button>
        </div>
      </form>
    </div>
