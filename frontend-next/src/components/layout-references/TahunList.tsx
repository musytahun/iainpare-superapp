...
import { GET_TAHUN, DELETE_TAHUN } from "@/graphql/references/tahun.graphql";
...
export default function TahunPage() {
  const { data, loading, error, refetch } = useQuery(GET_TAHUN);
  const [deleteTahun] = useMutation(DELETE_TAHUN);

  const handleDelete = async (id: number | string) => {
    if (!confirm("Apakah yakin ingin menghapus data ini?")) return;
    try {
      const res = await deleteTahun({ variables: { id: Number(id) } });
    ...
    <QueryHandler loading={loading} error={error} skeleton={<TableSkeleton rows={6} />}>
      <div>
        ...
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-slate-800 dark:text-white">Tahun</h1>
            <ActionButtons
              basePath="/references/tahun"
              entityName="Tahun"
              permissions={{ create: "tahun.create" }}
              showCreate
            />
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm overflow-hidden">
          <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
            ...
            <tbody className="bg-white dark:bg-slate-800/50 divide-y divide-slate-200 dark:divide-slate-700">
              {data?.getTahun?.map((getTahun: any, index: number) => (
                <tr
                  key={getTahun.id}
