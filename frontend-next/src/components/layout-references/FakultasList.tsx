...
import { GET_FAKULTAS, DELETE_FAKULTAS } from "@/graphql/references/fakultas.graphql";
...
export default function FakultasPage() {
  const { data, loading, error, refetch } = useQuery(GET_FAKULTAS);
  const [deleteFakultas] = useMutation(DELETE_FAKULTAS);

  const handleDelete = async (id: number | string) => {
    if (!confirm("Apakah yakin ingin menghapus data ini?")) return;
    try {
      const res = await deleteFakultas({ variables: { id: Number(id) } });
    ...
    <QueryHandler loading={loading} error={error} skeleton={<TableSkeleton rows={6} />}>
      <div>
        ...
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-slate-800 dark:text-white">Fakultas</h1>
            <ActionButtons
              basePath="/references/fakultas"
              entityName="Fakultas"
              permissions={{ create: "fakultas.create" }}
              showCreate
            />
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm overflow-hidden">
          <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
            ...
            <tbody className="bg-white dark:bg-slate-800/50 divide-y divide-slate-200 dark:divide-slate-700">
              {data?.getFakultas?.map((getFakultas: any, index: number) => (
                <tr
                  key={getFakultas.id}
