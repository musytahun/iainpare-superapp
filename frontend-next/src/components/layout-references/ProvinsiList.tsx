...
import { GET_PROVINSI, DELETE_PROVINSI } from "@/graphql/references/provinsi.graphql";
...
export default function ProvinsiPage() {
  const { data, loading, error, refetch } = useQuery(GET_PROVINSI);
  const [deleteProvinsi] = useMutation(DELETE_PROVINSI);

  const handleDelete = async (id: number | string) => {
    if (!confirm("Apakah yakin ingin menghapus data ini?")) return;
    try {
      const res = await deleteProvinsi({ variables: { id: Number(id) } });
    ...
    <QueryHandler loading={loading} error={error} skeleton={<TableSkeleton rows={6} />}>
      <div>
        ...
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-slate-800 dark:text-white">Provinsi</h1>
            <ActionButtons
              basePath="/references/provinsi"
              entityName="Provinsi"
              permissions={{ create: "provinsi.create" }}
              showCreate
            />
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm overflow-hidden">
          <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
            ...
            <tbody className="bg-white dark:bg-slate-800/50 divide-y divide-slate-200 dark:divide-slate-700">
              {data?.getProvinsi?.map((getProvinsi: any, index: number) => (
                <tr
                  key={getProvinsi.id}
